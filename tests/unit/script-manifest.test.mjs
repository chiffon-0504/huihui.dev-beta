...
describe("route-specific JavaScript manifests", () => {
  for (const family of routeFamilies) {
    test(`${family.name} has one exact feature manifest with ZH/EN/JA parity`, async () => {
      ...
      for (const [relativePath, route] of family.pages) {
        ...
        localizedManifests.push([...scripts].sort());
      }
    ...
    for (const family of routeFamilies) {
      for (const [relativePath, route] of family.pages) {
        ...
        for (const localeScript of [
          ...
          expectBefore(scripts, localeScript, "/js/i18n.js", relativePath);
        }
