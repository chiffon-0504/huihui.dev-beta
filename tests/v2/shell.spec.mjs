...
const locales = [
  { route: "/", lang: "zh-Hant", skip: "跳至主要內容", navigation: "主要導覽" },
  { route: "/en/", lang: "en", skip: "Skip to main content", navigation: "Main navigation" },
  { route: "/ja/", lang: "ja", skip: "メインコンテンツへ移動", navigation: "メインナビゲーション" },
];
...
for (const locale of locales) {
  for (const viewport of [{ width: 1440, height: 900 }, { width: 390, height: 844 }]) {
    test(`${locale.lang} shell and keyboard at ${viewport.width}px`, async ({ page, baseURL }, testInfo) => {
      ...
      await expect(nav.getByRole("link", { name: "Works", exact: true })).toHaveAttribute("href", "#works");
      ...
      await page.goto(locale.route);
      await page.keyboard.press("Tab");
      for (const label of ["huihui.dev", "Works", "About", "GitHub"]) {
        await page.keyboard.press("Tab");
        ...
        if (label === "Works" || label === "About") {
          await page.keyboard.press("Enter");
          await expect(page.locator(`#${label.toLowerCase()}`)).toBeFocused();
          // Reload instead of relying on engine-specific fragment tab order.
          await page.goto(locale.route);
          for (let index = 0; index < ["skip", "huihui.dev", "Works", "About"].indexOf(label) + 1; index++) {
            await page.keyboard.press("Tab");
          }
          await expect(current).toBeFocused();
        }
