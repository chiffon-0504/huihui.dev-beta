...
const hasFloatingSpace = (page: Page) => page.locator(".desktop").evaluate((node) =>
  node.clientWidth >= Math.ceil(320 + 320 / 0.42));
...
for (const locale of supportedLocales) {
  ...
  test(`${locale} canvas width controls first frame and classic-scrollbar transitions`, async ({ page }) => {
    const errors: string[] = [];
    ...
    await page.addInitScript(() => {
      const capture = () => {
        const node = document.querySelector<HTMLElement>(".desktop");
        if (!node) { requestAnimationFrame(capture); return; }
        document.documentElement.dataset.firstDesktopMode = node.classList.contains("is-compact") ? "compact" : "floating";
      };
      requestAnimationFrame(capture);
    });
    await page.goto(localeHref(locale));
    const canvas = page.locator(".desktop"), item = page.locator("#playing"), title = item.locator(".window-titlebar");
    await expect(page.locator("html")).toHaveAttribute("data-first-desktop-mode", "compact");
    expect(await page.evaluate(() => matchMedia("(min-width: 1131px)").matches)).toBe(true);
    expect(await canvas.evaluate((node) => node.clientWidth)).toBe(1067);
    ...
    const reachable = async () => {
      ...
      for (const target of [title, item.locator(".window-close")]) {
        expect(await target.evaluate((node) => {
          ...
          return [r.left + 2, r.left + 24, r.right - 2].every((x) =>
            node.contains(document.elementFromPoint(x, r.top + r.height / 2)));
        })).toBe(true);
      }
    };
    await reachable();
    // Test both sides of the real 320 + 320 / .42 = 1081.905px boundary.
    for (const width of [1081, 1082]) {
      await canvas.evaluate((node: HTMLElement, width) => { node.style.width = `${width}px`; }, width);
      await expect(item).toHaveCSS("position", width === 1081 ? "relative" : "absolute");
      await expect(title).toHaveAttribute("tabindex", width === 1081 ? "-1" : "0");
      await reachable();
    }
    ...
    for (const width of [1067, 1082, 1067, 1082]) {
      await canvas.evaluate((node: HTMLElement, width) => { node.style.width = `${width}px`; }, width);
      await expect(item).toHaveCSS("position", width === 1067 ? "relative" : "absolute");
      if (width === 1082) await expect.poll(() => position(item)).toEqual(moved);
      await reachable();
    }
    await item.locator(".window-close").click();
    await expect(item).toHaveCount(0);
    // Also catch an incorrect compact first frame on a sufficiently wide canvas.
    await page.unroute("**/*");
    await page.setViewportSize({ width: 1440, height: 900 });
    await page.goto(localeHref(locale));
    await expect(page.locator("html")).toHaveAttribute("data-first-desktop-mode", "floating");
  ...
  for (const width of [1440, 1131, 768, 390]) {
    test(`${locale} desktop windows, controls and themes at ${width}px`, async ({ page, baseURL }, testInfo) => {
      ...
      for (const item of await page.locator(".desktop-window").all()) {
        ...
        await expect(item).toHaveCSS("position", await hasFloatingSpace(page) ? "absolute" : "relative");
      }
      ...
      if ((width === 1131 || width === 1440) && await hasFloatingSpace(page)) {
  ...
  test(`${locale} compact layout disables title bar keyboard movement and restores desktop focusability`, async ({ page }) => {
    await page.setViewportSize({ width: 1440, height: 900 });
    await page.goto(localeHref(locale));
    await page.locator(".desktop").evaluate((node: HTMLElement) => { node.style.width = "1082px"; });
    await page.evaluate(() => document.addEventListener("keydown", (event) => {
      document.documentElement.dataset.keyPrevented = String(event.defaultPrevented);
    }));
    const item = page.locator("#playing"), title = item.locator(".window-titlebar");
    ...
    await page.keyboard.press("ArrowDown");
    const moved = await position(item);
    for (const width of [1081, 1067, 720, 342]) {
      await page.locator(".desktop").evaluate((node: HTMLElement, width) => { node.style.width = `${width}px`; }, width);
...
test("window boundaries survive dragging and viewport mode changes", async ({ page }) => {
  ...
  for (const width of [1440, 700, 390, 900]) {
    ...
    await expect(playing).toHaveCSS("position", await hasFloatingSpace(page) ? "absolute" : "relative");
